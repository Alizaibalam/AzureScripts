...
function SetDiagnostics {
    ...
    $cfgPath = $null
   # $storage = Get-AzureRmStorageAccount -ResourceGroupName diagnostics
...
Write-Host("Found $subscriptionsCount subscriptions test1")
...
foreach ($subscription in $subscriptions){
	...
	try {
        ...
        $storages = @{}
        Write-Host (" ********* list of Classic VMs found ********* ")
        $vms
        Write-Host (" ********* list of Classic VMs found end ********* ")
		...
		foreach ($vm in $vms){
            ...
            Write-Host ( " creating VMresult Object using '$vm'" )
